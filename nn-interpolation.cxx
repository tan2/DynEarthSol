...
namespace {
    ...
    void inject_field(const int_vec &idx, const double_vec &source, double_vec &target)
    {
    ...
    void inject_field(const int_vec &idx, const tensor_t &source, tensor_t &target)
    {
        #pragma omp parallel for default(none)          \
            shared(idx, source, target)
        for (std::size_t i=0; i<target.size(); i++) {
    ...
    void nn_interpolate_elem_fields(Variables &var, const int_vec &idx)
    {
