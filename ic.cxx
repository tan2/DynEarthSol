    ...
    case 2:
        {
            ...
            const double t_top = param.bc.surface_temperature;
            const double t_bot = param.bc.mantle_temperature;
            const double z_bot = param.mesh.zlength;
            double rhoh0 = dens_c * hs;

            // from book "Geodymanics Second Edition" by Turcotte and Schubert
            // heat flux generated by radiogenic heat production
            // with H = H0 * exp(-z/hr)
            double qh = rhoh0 * pow(hr,2) * (1-exp(-z_bot/hr))/z_bot; // W/m^2
            double q = (t_bot-t_top)/z_bot * cond_c;
            double qm = q - qh;

            for (int i=0; i<var.nnode; ++i) {
                double y = -(*var.coord)[i][NDIMS-1];
                temperature[i] = t_top + qm/cond_c*y + rhoh0*pow(hr,2)/cond_c*(1-exp(-y/hr));
            }
            break;
        }
    case 90:
