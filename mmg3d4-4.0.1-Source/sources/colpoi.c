...
#ifdef REQUIRED
    if ( mesh->tetra[kk].tag & M_REQUIRED ) return(0);
#endif
   }
  crit *= coef; 
  //assert(crit < 1e20);
...
int MMG_colpoi2(pMesh mesh,pSol sol,int iel,int ia,int ib,double coef) {
  ...
  crit *= coef;
  //sinon on pourrait creer des invalides!!!!!
  if(mesh->info.optles) 
...
//printf("bad quality : %f > %f\n",cal,crit);
    ...
    for (i=0; i<3; i++) {
      ...
      mb   = &sol->met[iadr];
       if ( ppt->mark < base ) {
        len = MMG_length(ca,cb,ma,mb);
	if ( len > LLONG /*1.51*/ ) { //10/02/14 a voir si on laisse LLONG
	        MMG_nlen++;
          memcpy(pb->c,coor,3*sizeof(double));
          memcpy(&sol->met[iadb],solu,sol->offset*sizeof(double));
