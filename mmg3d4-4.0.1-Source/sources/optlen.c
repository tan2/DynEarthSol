  ...
  do {
   ...
   //	else if ( pt->flag != base - 1 ) continue;
	...
	tmp3 = 0;
	  ...
	  /*len = MMG_length(&(mesh->point[pt1->v[MMG_idir[nk][0]]].c[0]),
	    &(mesh->point[pt1->v[MMG_idir[nk][1]]].c[0]),&hp,&hb); 
	    len += MMG_length(&(mesh->point[pt1->v[MMG_idir[nk][0]]].c[0]),
	    &(mesh->point[pt1->v[MMG_idir[nk][2]]].c[0]),&hp,&hb); 
	    len += MMG_length(&(mesh->point[pt1->v[MMG_idir[nk][1]]].c[0]),
	    &(mesh->point[pt1->v[MMG_idir[nk][2]]].c[0]),&hp,&hb); 
	    len /= 3.;
	    len = 1./len;
	    len *=  MMG_length(ca,cb,&hp,&hb); */  	
...
/* optimise using heap : point position on normal for LES criterion*/
int MMG_optlen_LES(pMesh mesh,pSol sol,double declic,int base) {
  pTetra    pt,pt1;
  pPoint    ppa,ppb,p1,p2,p3;
  pQueue    queue;
  List      list;
...
assert(ctg < 2);
      ...
      do {
        ...
        for (l=1; l<=lon; l++) {
          ...
          cal = MMG_caltet_LES(mesh,sol,iel); 
//printf("l %d cal %e %e\n",l,cal,MMG_rao(mesh,iel,0));
          if ( cal > ctg )  break;
          list.qual[l] = cal;
        }
