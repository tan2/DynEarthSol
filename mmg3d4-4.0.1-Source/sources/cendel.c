...
int MMG_cendel_LES(pMesh mesh,pSol sol,double declic,int base) {
  pTetra	pt,pt1;
  pQueue	queue;
  List		list;
  double	crit;
  ...
  do {
   ...
   if(pt->tag & M_REQUIRED) continue;
    ...
    for (i=0; i<6; i++) { 
      ...
      /* qual crit */
      crit = MMG_caltet_LES(mesh,sol,k); 
      for (j=2; j<=lon; j++) {
        jel  = list.tetra[j] / 6;
        pt1  = &mesh->tetra[jel];
        crit = M_MAX(crit,MMG_caltet_LES(mesh,sol,jel));
	if(crit > 1) break;
#ifdef REQUIRED
	if(pt1->tag & M_REQUIRED) break;
#endif
      ...
      crit *= SCRIT;  
			//MMG_outqua(mesh,sol);
      //printf("on envoie swapar avec %d : %e %e\n",lon,crit,declic);
      ier = MMG_swapar(mesh,sol,queue,&list,lon,crit,declic); 
      if ( ier > 0 ) {
        ns++; 
        break;
