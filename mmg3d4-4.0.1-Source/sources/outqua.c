...
int MMG_outqua(pMesh mesh,pSol sol) {
  ...
  for (k=1; k<=mesh->ne; k++) {
    ...
    rap4 = coef * dcal; 
    if(dcal == CALLIM) {
      fprintf(stdout,"WARNING : Wrong elt %d : %d %d %d %d ----> bad orientation ?? vol = %e\n",k,pt->v[0],pt->v[1],pt->v[2],pt->v[3],MMG_voltet(mesh,k));
      tmp = pt->v[2];
      pt->v[2] = pt->v[3];
      pt->v[3] = tmp;
      dcal = MMG_caltet(mesh,sol,k);//(sol->offset==6) ? MMG_caltet_ani(mesh,sol,k) : MMG_caltet_iso(mesh,sol,k);             
...
/* Store in eltab elements which have worse quality than rapman,
 * eltab is allocated and could contain mesh->ne elements */
int MMG_searchqua(pMesh mesh,pSol sol, double rapmax, int * eltab) {
  pTetra    pt;
  double    coef,rap4,rapl,dcal;
  int       his10[11],his01[33],rapnum,iout;
  int       k,i,j,imax,iel,ir,nn,nex,ielreal,tmp;
...
/* print mesh quality histo */
int MMG_outquaLES(pMesh mesh,pSol sol) {
  pTetra    pt;
  double    coef,rap4,rapl,rapmin,rapmax,rapavg,dcal;
  int       his10[11],his01[11],rapnum,iout;
  int       k,i,j,imax,iel,ir,nn,nex,ielreal,tmp;
  ...
  for (k=1; k<=mesh->ne; k++) {
    ...
    nn++;     
    dcal = MMG_caltet_LES(mesh,sol,k); 
    if(dcal > 1) {
      printf("argggggg %d %e %e\n",k,dcal,MMG_calte1_iso(mesh,sol,k)*ALPHAD);
      exit(0);
      dcal = 1.;
    }   
    ir   = (int)(dcal*10);
  ...
  if(rapmax > 0.9) 
    ...
    imax = M_MIN((int)((rapmax)*10),8);  
    
  for (i=M_MAX((int)(rapmin*10),1); i<M_MIN(imax,5); i++) {
    his10[5] += his10[i];
  }
  fprintf(stdout,"     %5.4f < Q < %5.5f   %7d   %6.2f %%\n",
	  0.,0.5,his10[5],100.*(his10[5]/(float)his01[0]));
  for (i=6; i<=imax; i++) {
    fprintf(stdout,"     %5.4f < Q < %5.5f   %7d   %6.2f %%\n",
	    (i)/10.,(i+1)/10.,his10[i],100.*(his10[i]/(float)his01[0]));
  }

  /* quality per interval */   
  if (j != 0) {
    fprintf(stdout,"\n");
    imax = (int)(M_MAX(1,((rapmax-0.9)*100)));
    imax = imax+1;
    for (i=1; i<imax; i++) {
      fprintf(stdout,"     %5.4f < Q < %5.4f   %7d   %6.2f %%\n",
	      (i-1)/100.+0.9,(i)/100.+0.9,his01[i],100.*(his01[i]/(float)his01[0]));
    }
    fprintf(stdout,"     %5.4f < Q            %7d   %6.2f %%\n",
	      (imax-1)/100.+0.9,his01[imax],100.*(his01[imax]/(float)his01[0]));
  }
  return(iout);
}
