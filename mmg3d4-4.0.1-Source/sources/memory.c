...
void *M_malloc(size_t size,char *call) {
  ...
  if ( stack < MAXMEM ) {
    ...
    fprintf(stderr,"M_malloc: unable to store %10lud bytes pointer. table full\n",
	    size);
    return(0);
  }
}
...
void *M_calloc(size_t nelem, size_t elsize,char *call) {
  ...
  if ( stack < MAXMEM ) {
    ...
    fprintf(stderr,"M_calloc: unable to allocate %10lu bytes. table full\n",
	    nelem*elsize);
    return(0);
  }
}
...
/* Deallocates left allocated spaces. */
void MMG_freeAll() {
  int   i;
  ...
  for (i=1; i<=MAXMEM; i++) {
    if (mstack[i].ptr) {
      ...
      cur = i;
#ifdef MEMDEBUG
      fprintf(stdout,"M_freeAll: deallocate %p of size %d       (%d,%d)\n",
	      ptr,mstack[i].size,stack,cur);
#endif
    }
  }
