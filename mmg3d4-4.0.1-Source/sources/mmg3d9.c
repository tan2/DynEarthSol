  ...
  */
  ...
  for (k=1; k<=mesh->np; k++) {
    ...
    if ( ppt->tag & M_MOVE ) {
      ...
      if(MMG_computeMetric(mesh,sol,k,coor) == -1)
	;//printf("point %d not found\n",k);
...
int MMG_chkmov(pMesh mesh,char level) {
  ...
  for (k=1; k<=mesh->np; k++) {
    ppt = &mesh->point[k];
    if ( ppt->tag & M_MOVE ) {
      if ( pd->alpha[k] < SHRT_MAX )  return(0);
      ppt->tag &= ~M_MOVE;
      nc++;
    }
  }
...
int MMG_mmg3d9(pMesh mesh,pSol sol,int *alert) {
  ...
  if (  MMG_dikomv(mesh,sol,t) ) {
    ...
    while (alpha < SHRT_MAX && iter < maxiter) {
      t = SHRT_MAX - alpha;
      i = 0;
      do {
