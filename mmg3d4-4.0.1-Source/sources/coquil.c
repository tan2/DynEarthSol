...
/*coquil even if ia is bdry edge*/
int MMG_coquilgen(pMesh mesh,int iel,int ia,pList list) {
  pTetra      pt;
  int        *adja,i,iadr,adj,base,na,nb,ipa,ipb,piv,ilist,kref;
  ...
  /* At this point, the first travel, in one direction, of the shell is complete. Now, analyze why
     the travel ended. */
  if ( adj == iel )  return(2*ilist);
  assert(!adj); // a boundary has been detected

  adj = list->tetra[ilist] / 6;
  i   = list->tetra[ilist] % 6;
  ilist = 1;
  ...
  /* overflow */
  if ( ilist > LONMAX-3 ) {
    fprintf(stdout,"  ## Warning: problem in coquil process.");
    return(-1);
  }

  adja = &mesh->adja[4*(adj-1)+1];
  if ( pt->v[ MMG_ifar[i][0] ] == piv ) {
    adj = adja[ MMG_ifar[i][0] ] / 4;
    piv = pt->v[ MMG_ifar[i][1] ];
  }
  else {
    adj = adja[ MMG_ifar[i][1] ] /4;
    piv = pt->v[ MMG_ifar[i][0] ];
  }
