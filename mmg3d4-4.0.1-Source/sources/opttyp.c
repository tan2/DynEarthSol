...
int MMG_opttyp(pMesh mesh,pSol sol,double declic,int *alert,int base) {
  pTetra     pt,pt1;
  pPoint     pa,pb;
  List       list;
  pQueue     queue;
  ...
  int       *adja,i,k,l,iar,ia,ib,iel,iadr,ier,item[2],nd,ityp,np;
  ...
  do {
	...
	*alert = 1;
	...
	len = MMG_length(ca,cb,ma,mb);
	if(!mesh->info.noinsert && len > LLONG2)
	  ier = MMG_spledg(mesh,sol,queue,&list,lon,crit,declic);
  ...
  if (!mesh->info.noswap) {
    *alert = 0;
    ndd     = MMG_cendel(mesh,sol,declic,-1);
