...
#ifdef TRILIBRARY
      end1 = segmentlist[index0++];
      end2 = segmentlist[index0++];
      if (segmentmarkers) {
...

{
  ...
  while (subsegloop.ss != (subseg *) NULL) {
    ...
    slist[index0++] = vertexmark(endpoint1);
    slist[index0++] = vertexmark(endpoint2);
    if (!b->nobound) {
      /* Copy the boundary marker. */
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      if ((triangleloop.tri < trisym.tri) || (trisym.tri == m->dummytri)) {
        ...
        dest(triangleloop, p2);
#ifdef TRILIBRARY
        elist[index0++] = vertexmark(p1);
        elist[index0++] = vertexmark(p2);
#endif /* TRILIBRARY */
        if (b->nobound) {
#ifndef TRILIBRARY
...
#ifdef TRILIBRARY
    nlist[index0++] = neighbor1;
    nlist[index0++] = neighbor2;
    nlist[index0++] = neighbor3;
#else /* not TRILIBRARY */
